...
ENTRYPOINT java -Dcurrent.node.name=$CURRENT_NODE -Dbasic.metrics.mechanism=$RETRIEVAL_TYPE -Dprometheus.base.url=$PROMETHEUS_URL -Dimport.task.fixedDelay.in.milliseconds=$IMPORT_DELAY -Dch.connection.string=$CH_HOST -Dsettings.mongo.uri=$MONGO_CONNECTION_STRING -jar /telemetry-broker-1.0.jar